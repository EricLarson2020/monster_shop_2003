...
RSpec.describe Cart do
  describe 'Instance Methods' do
    before :each do
      ...
      @giant = @megan.items.create!(name: 'Giant', description: "I'm a Giant!", price: 50, image: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTaLM_vbg2Rh-mZ-B4t-RSU9AmSfEEq_SN9xPP_qrA2I6Ftq_D9Qw', inventory: 8 )
    ...
    it '.add_quantity' do
      @cart.add_quantity(5, @giant, @giant.inventory)
    ...
    it '.remove_quantity' do
      ...
      the_cart.remove_quantity(1, @giant)
      expect(the_cart.contents).to eq({
        @ogre.id.to_s => 5,
        @giant.id.to_s => 2
        })
    end

    it '.remove_quantity(FALSE)' do
      @cart.remove_quantity(5, @ogre)
      expect(@cart.contents).to eq({
        @ogre.id.to_s => 1,
        @giant.id.to_s => 2
        })
